document.addEventListener('DOMContentLoaded', () => {
    ...
    const fallbackExpozice = [
        { nazev: 'Dřevo a barva', kategorie: 'Sochařství', popis: 'Sbírka impresionistických výtvorů ze dřeva zachycujících hru světla v různých denních dobách.', obrazek: 'img/druhy.jpg' },
        { nazev: 'Barvy v kaňkách', kategorie: 'Abstraktní', popis: 'Abstraktní a novodobé díla.', obrazek: 'img/prvni.jpg' },
        { nazev: 'Červenající se tváře', kategorie: 'Portréty', popis: 'Moderní obrazy portrétů lidí po celém světě, ale i vymyšlených.', obrazek: 'img/treti.jpg' },
        { nazev: 'Vysypané fotky', kategorie: 'Fotografie', popis: 'Výběr dokumentárních fotografií a obrazů zachycujících klíčové momenty druhé poloviny 20. století.', obrazek: 'img/ctvrty.jpg' },
        { nazev: 'Momenty z minulosti', kategorie: 'Klasické', popis: 'Obrazy předevšem z olejových barev, často zařazovány mezi 19. – 20. století.', obrazek: 'img/paty.jpg' },
        { nazev: 'Zlaté ručičky', kategorie: 'Sochařství', popis: 'Textilní, dřevěné a kamené umění jako médium pro vyprávění příběhů a kulturní paměti.', obrazek: 'img/sesty.jpg' },
    ];
    ...
    async function nactiExpozice() {
        ...
        try {
            ...
            console.error('Chyba při načítání expozic, používám záložní data:', chyba);

            window._expoziceData = fallbackExpozice;
            expoziceContainer.innerHTML = fallbackExpozice.map(vytvorKartuExpozice).join('');
            inicializujReveal();
        }
    }
